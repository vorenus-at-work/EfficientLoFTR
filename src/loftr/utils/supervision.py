...
def spvs_coarse(data, config):
    ...
    # N = batch size
    # _ = number of channels (usually 3 for RGB)
    # H0, W0 = height and width of first image
    # H1, W1 = height and width of second image
    N, _, H0, W0 = data['image0'].shape
    _, _, H1, W1 = data['image1'].shape
    # Get the coarse-level resolution scale factor from config
    # This determines how much to downsample the images for coarse matching
    scale = config['LOFTR']['RESOLUTION'][0]
    # Apply additional scaling if provided in the data
    # scale0 and scale1 handle cases where images might have different scales
    # [:, None] adds an extra dimension to make broadcasting work correctly
    scale0 = scale * data['scale0'][:, None] if 'scale0' in data else scale
    scale1 = scale * data['scale1'][:, None] if 'scale1' in data else scale
    # Calculate the dimensions at the coarse level by dividing by scale
    # h0, w0 = dimensions of first image at coarse level
    # h1, w1 = dimensions of second image at coarse level
    h0, w0, h1, w1 = map(lambda x: x // scale, [H0, W0, H1, W1])

    # 2. warp grids
    # create kpts in meshgrid and resize them to image resolution
    # create_meshgrid(h0, w0, ...):
    # For h0=2, w0=3, creates:
    # (0,0) (1,0) (2,0)
    # (0,1) (1,1) (2,1)
    # reshape(1, h0*w0, 2):
    # Flattens to:
    ...
    # repeat(N, 1, 1):
    # Repeats each grid point N times along the batch dimension
    # For N=2, creates:
    # Batch 1: [(0,0), (1,0), (2,0), (0,1), (1,1), (2,1)]
    # Batch 2: [(0,0), (1,0), (2,0), (0,1), (1,1), (2,1)]
    grid_pt0_c = create_meshgrid(h0, w0, False, device).reshape(1, h0*w0, 2).repeat(N, 1, 1)    # [N, hw, 2]
    # Scale the coarse grid points to original image resolution
    grid_pt0_i = scale0 * grid_pt0_c  # [N, h0*w0, 2]
    ...
    # 3. check if mutual nearest neighbor (MNN)
    # round the warped kpts to the nearest integer
...
@torch.no_grad()
def spvs_coarse_satellite(data, config):
    """
    Supervision for satellite images using homography transformation
    Explaination of the following codes could be found in the above function spvs_coarse.
    
    Args:
        data (dict): Must contain:
            - image0, image1: Image pairs
            - H_0to1, H_1to0: Homography matrices
    """
    # 1. misc
    device = data['image0'].device
    N, _, H0, W0 = data['image0'].shape
    ...
    w_pt0_i = warp_kpts_satellite(grid_pt0_i, data['H_0to1'])
    w_pt1_i = warp_kpts_satellite(grid_pt1_i, data['H_1to0'])
    w_pt0_c = w_pt0_i / scale1
    w_pt1_c = w_pt1_i / scale0

    # 3. check mutual nearest neighbor
    w_pt0_c_round = w_pt0_c.round()
    if config.LOFTR.LOSS.COARSE_OVERLAP_WEIGHT:
        w_pt0_c_error = (1.0 - 2*torch.abs(w_pt0_c - w_pt0_c_round)).prod(-1)
    w_pt0_c_round = w_pt0_c_round.long()
    ...
    # 4. construct ground truth confidence matrix
    conf_matrix_gt = torch.zeros(N, h0*w0, h1*w1, device=device)
    b_ids, i_ids = torch.where(correct_0to1 != 0)
    j_ids = nearest_index1[b_ids, i_ids]
...
def compute_supervision_coarse(data, config):
    ...
    elif data_source.lower() == 'satellite':
        compute_supervision_coarse_satellite(data, config)
    else:
        raise ValueError(f'Unknown data source: {data_source}')

def compute_supervision_coarse_satellite(data, config):
    """
    A wrapper function that calls spvs_coarse_satellite with error handling.
    
    Args:
        data (dict): Input data containing image pairs and homography matrices
        config: Configuration object
    """
    try:
        spvs_coarse_satellite(data, config)
    except Exception as e:
        raise ValueError(f'Error in coarse-level supervision for satellite images: {str(e)}')
...
def compute_supervision_fine(data, config, logger=None):
    ...
    if data_source.lower() in ['scannet', 'megadepth']:
        ...
        spvs_fine_satellite(data, config, logger)
    else:
        raise NotImplementedError

@static_vars(counter = 0)
@torch.no_grad()
def spvs_fine_satellite(data, config, logger = None):
    """
    Update:
        data (dict):{
            "expec_f_gt": [M, 2], used as subpixel-level gt
    ...
    if m == 0:  # special case: there is no coarse gt
        ...
        # Create fine-level grid points around coarse matches
        grid_pt0_f = create_meshgrid(hf0, wf0, False, device) - W // 2 + 0.5  # [1, hf0, wf0, 2]
        grid_pt0_f = rearrange(grid_pt0_f, 'n h w c -> n c h w')
        
        # Unfold local windows
        ...
        # Batch homography warping
        w_pt0_i = warp_kpts_satellite(grid_pt0_f_unfold, data['H_0to1'][b_ids])
        correct_0to1_f = torch.ones_like(correct_0to1_f, dtype=torch.bool)  # All points are valid for homography
        
        # Calculate the offset from the matched point
        delta_w_pt0_i = w_pt0_i - pt1_i[b_ids, j_ids][:,None,:]  # [m, WW, 2]
        delta_w_pt0_f = delta_w_pt0_i / scalei1[:,None,:] + W // 2 - 0.5
        delta_w_pt0_f_round = delta_w_pt0_f[:, :, :].round()
        ...
        m_ids, i_ids = torch.where(correct_0to1_f != 0)
        j_ids = nearest_index1[m_ids, i_ids]  # i_ids, j_ids range from [0, WW-1]
        j_ids_di, j_ids_dj = j_ids // W, j_ids % W  # Get the (i, j) index in fine windows
        m_ids, i_ids, j_ids_di, j_ids_dj = m_ids.to(torch.long), i_ids.to(torch.long), j_ids_di.to(torch.long), j_ids_dj.to(torch.long)
        ...
        # Construct a pixel-level gt conf_matrix
        conf_matrix_f_gt = torch.zeros(m, WW, WW, device=device, dtype=torch.bool)
        conf_matrix_f_gt[m_ids, i_ids, j_ids] = 1
        data.update({'conf_matrix_f_gt': conf_matrix_f_gt})
...
def compute_supervision_fine_satellite(data, config, logger=None):
    """
    A wrapper function that calls spvs_fine_satellite with error handling and logging.
    ...
    try:
        ...
        if logger:
            logger.error(f"Error in fine-level supervision for satellite images: {str(e)}")
        raise e